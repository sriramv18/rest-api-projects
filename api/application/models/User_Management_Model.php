...
class User_Management_Model extends SPARQ_Model {
   ...
   public function getUserDetails($userid){
	   ...
	   $this->db->from(USERPROFILE.' as USERPROFILE');
	   $this->db->join(ENTITY.' as ENTITY','USERPROFILE.fk_entity_id = ENTITY.entity_id','LEFT');
	   $this->db->join(DESIGNATION.' as DESIGNATION','USERPROFILE.fk_designation = DESIGNATION.designation_id ','LEFT');
	   $this->db->join(STATE.' as STATE','USERPROFILE.fk_state = STATE.state_id','LEFT');
	   $this->db->join(CITY.' as CITY','USERPROFILE.fk_city = CITY.city_id','LEFT');
	   $this->db->join(USERPROFILE.' as USERPROFILE1','USERPROFILE.fk_createdby = USERPROFILE1.userid');
	   $this->db->join(USERPROFILE.' as USERPROFILE2','USERPROFILE.fk_updatedby = USERPROFILE2.userid','LEFT');
	   $this->db->join(USERPROFILEHIERARCHY.' as USERPROFILEHIERARCHY','USERPROFILE.userid = USERPROFILEHIERARCHY.fk_user_id','LEFT');
	   $this->db->join(LENDERHIERARCHY.' as LENDERHIERARCHY','USERPROFILEHIERARCHY.user_lender_hierarchy_id = LENDERHIERARCHY.lender_hierarchy_id','LEFT');
	   $this->db->join(USERPROFILEROLES.' as USERPROFILEROLES','USERPROFILE.userid = USERPROFILEROLES.fk_userid','LEFT');
	   $this->db->join(ENTITYTYPE.' as ENTITYTYPE','ENTITY.fk_entity_type_id = ENTITYTYPE.entity_type_id ','LEFT');
